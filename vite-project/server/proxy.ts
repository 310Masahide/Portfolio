...
type PromptProxyRouteOptions = {
  ...
  /** API キーとプロンプトで upstream を呼ぶ（キー検証後に渡される） */
  fetchUpstream: (apiKey: string, prompt: string) => Promise<Response>
}
...
function mountPromptProxyRoute(
  ...
  middlewares.use(path, async (req, res) => {
    ...
    const apiKey = requireAsciiApiKey(res, normalizeEnvValue(env[opts.envKey] ?? ''), {
      missingMessage: opts.missingMessage,
      nonAsciiMessage: opts.nonAsciiMessage,
    })
    if (!apiKey) return
    const upstream: PromptUpstream = (prompt) => opts.fetchUpstream(apiKey, prompt)
