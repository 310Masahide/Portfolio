...
/** フィルタ後の日付キー（新しい順）。`sortedDateKeys` を渡すと初回の keys+sort を省略 */
export function filterSortedDates(
  entries: FurikaeriEntriesMap,
  opt: HistoryFilterOptions,
  sortedDateKeys?: string[],
): string[] {
  const all = sortedDateKeys ?? Object.keys(entries).sort((a, b) => b.localeCompare(a))
